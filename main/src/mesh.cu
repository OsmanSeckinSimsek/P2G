...
template<typename T>
__global__ void countPositiveWeightsKernel(const T* weights, int n, int* count)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    if (weights[idx] > T(0)) atomicAdd(count, 1);
}

template<typename T>
__global__ void packSPHTargetKernel(int targetRank,
                                    const T* weights, const T* weightedVx, const T* weightedVy, const T* weightedVz,
                                    int n, int baseOffset, int* writeOffset,
                                    int* outRanks, uint64_t* outIndices,
                                    T* outWeights, T* outWeightedVx, T* outWeightedVy, T* outWeightedVz)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    T w = weights[idx];
    ...
    int pos = atomicAdd(writeOffset, 1);
    int out = baseOffset + pos;
    outRanks[out]      = targetRank;
    outIndices[out]    = static_cast<uint64_t>(idx);
    outWeights[out]    = w;
    outWeightedVx[out] = weightedVx[idx];
    outWeightedVy[out] = weightedVy[idx];
    outWeightedVz[out] = weightedVz[idx];
}

template<typename T>
__global__ void packSphByRankKernel(const int* remoteRanks, const uint64_t* remoteIndices,
                                    const T* remoteWeights,
                                    const T* remoteWeightedVx, const T* remoteWeightedVy, const T* remoteWeightedVz,
                                    int n, const int* sendDisp, int* rankWriteOffset,
                                    SphMsg<T>* sendPacked)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    sendPacked[out] = SphMsg<T>{remoteIndices[idx], remoteWeights[idx],
                                remoteWeightedVx[idx], remoteWeightedVy[idx], remoteWeightedVz[idx]};
}
...
template<typename T>
__global__ void updateMeshDistancePackedKernel(const NearMsg<T>* msgs, int count, T* meshDistance)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count) return;

    uint64_t meshIndex = msgs[idx].index;
    T        distance  = msgs[idx].distance;
...
template<typename T>
__global__ void assignVelocitiesPackedKernel(const NearMsg<T>* msgs, int count,
                                             T* meshVelX, T* meshVelY, T* meshVelZ, T* meshDistance)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count) return;
    ...
    uint64_t meshIndex = msgs[idx].index;
    unsigned long long int stored = *(unsigned long long int*)&meshDistance[meshIndex];
    if (stored == __double_as_longlong(msgs[idx].distance))
    {
        meshVelX[meshIndex] = msgs[idx].vx;
        meshVelY[meshIndex] = msgs[idx].vy;
        meshVelZ[meshIndex] = msgs[idx].vz;
    }
}

template<typename T>
__global__ void accumulateCellAvgPackedKernel(const CellAvgMsg<T>* msgs, int count,
                                              T* velXSum, T* velYSum, T* velZSum, int* cellCount)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count) return;
    uint64_t cell = msgs[idx].index;
    atomicAdd(&velXSum[cell], msgs[idx].vx);
    atomicAdd(&velYSum[cell], msgs[idx].vy);
    atomicAdd(&velZSum[cell], msgs[idx].vz);
    atomicAdd(&cellCount[cell], 1);
}

template<typename T>
__global__ void accumulateSPHPackedKernel(const SphMsg<T>* msgs, int count,
                                          T* meshWeightSum, T* meshWeightedVelX,
                                          T* meshWeightedVelY, T* meshWeightedVelZ)
{
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= count) return;
    uint64_t cell = msgs[idx].index;
    atomicAdd(&meshWeightSum[cell], msgs[idx].weight);
    atomicAdd(&meshWeightedVelX[cell], msgs[idx].wvx);
    atomicAdd(&meshWeightedVelY[cell], msgs[idx].wvy);
    atomicAdd(&meshWeightedVelZ[cell], msgs[idx].wvz);
}
                                          ...
                                          int powerDim)
{
    ...
    NearMsg<T>* d_recvNearPacked = nullptr;
    std::vector<NearMsg<T>> sendNearPacked, recvNearPacked;
    ...
    {
        int sendTotal = mesh.send_disp[mesh.numRanks_];
        sendNearPacked.resize(sendTotal);
        for (int i = 0; i < mesh.numRanks_; i++)
        {
            for (int j = mesh.send_disp[i]; j < mesh.send_disp[i + 1]; j++)
            {
                int local = j - mesh.send_disp[i];
                sendNearPacked[j] = NearMsg<T>{mesh.vdataSender[i].send_index[local],
                                               mesh.vdataSender[i].send_distance[local],
                                               mesh.vdataSender[i].send_vx[local],
                                               mesh.vdataSender[i].send_vy[local],
                                               mesh.vdataSender[i].send_vz[local]};
            }
        }

        recvNearPacked.resize(recvTotal);
        MPI_Datatype nearMsgType;
        MPI_Type_contiguous(sizeof(NearMsg<T>), MPI_BYTE, &nearMsgType);
        MPI_Type_commit(&nearMsgType);

        MPI_Alltoallv(sendNearPacked.data(), mesh.send_count.data(), mesh.send_disp.data(), nearMsgType,
                      recvNearPacked.data(), mesh.recv_count.data(), mesh.recv_disp.data(), nearMsgType,
                      MPI_COMM_WORLD);
        MPI_Type_free(&nearMsgType);

        if (recvTotal > 0)
        {
            checkCudaError(cudaMalloc(&d_recvNearPacked, recvTotal * sizeof(NearMsg<T>)), "Allocating d_recvNearPacked");
            checkCudaError(cudaMemcpy(d_recvNearPacked, recvNearPacked.data(), recvTotal * sizeof(NearMsg<T>), cudaMemcpyHostToDevice),
                           "Copying packed nearest recv to device");
        }
    }
    ...
    if (recvTotal > 0)
    {
        ...
        if (d_recvNearPacked)
            updateMeshDistancePackedKernel<T><<<recvBlocks, threadsPerBlock>>>(d_recvNearPacked, recvTotal, d_meshDistance);
        else
        ...
        if (d_recvNearPacked)
            assignVelocitiesPackedKernel<T><<<recvBlocks, threadsPerBlock>>>(
                d_recvNearPacked, recvTotal, d_meshVelX, d_meshVelY, d_meshVelZ, d_meshDistance);
        else
        ...
        if (d_recvNearPacked) cudaFree(d_recvNearPacked);
        if (d_recvIndices) cudaFree(d_recvIndices);
        if (d_recvDistances) cudaFree(d_recvDistances);
        if (d_recvVx) cudaFree(d_recvVx);
        if (d_recvVy) cudaFree(d_recvVy);
        if (d_recvVz) cudaFree(d_recvVz);
    }
                                         ...
                                         std::vector<T> h, int powerDim)
{
    ...
    for (int targetRank = 0; targetRank < mesh.numRanks_; targetRank++)
    {
        ...
        const bool useFullGpuPack = mesh.useCudaAwareMpi_ && mesh.useCudaAwareGpuPack_;

        std::vector<T> h_remoteWeightSum;
        std::vector<T> h_remoteWeightedVelX;
        std::vector<T> h_remoteWeightedVelY;
        std::vector<T> h_remoteWeightedVelZ;
        if (!useFullGpuPack)
        {
                h_remoteWeightSum.resize(maxTargetInboxSize);
                h_remoteWeightedVelX.resize(maxTargetInboxSize);
                h_remoteWeightedVelY.resize(maxTargetInboxSize);
                h_remoteWeightedVelZ.resize(maxTargetInboxSize);
        }

        int*      d_remoteRanksSPH      = nullptr;
        uint64_t* d_remoteIndicesSPH    = nullptr;
        T *       d_remoteWeightsSPH    = nullptr, *d_remoteWeightedVxSPH = nullptr,
                            *d_remoteWeightedVySPH = nullptr, *d_remoteWeightedVzSPH = nullptr;
        std::vector<int> targetNnz(mesh.numRanks_, 0);
        std::vector<int> targetOffset(mesh.numRanks_ + 1, 0);
        int* d_positiveCount = nullptr;
        int* d_singleWriteOffset = nullptr;
    ...
    if (useFullGpuPack)
    {
        if (maxTargetInboxSize > 0)
        {
            checkCudaError(cudaMalloc(&d_positiveCount, sizeof(int)), "Allocating d_positiveCount SPH");
            checkCudaError(cudaMalloc(&d_singleWriteOffset, sizeof(int)), "Allocating d_singleWriteOffset SPH");
        }

        // Pass 1: count non-zero aggregated cells per target rank.
        for (int targetRank = 0; targetRank < mesh.numRanks_; targetRank++)
        {
            if (targetRank == mesh.rank_) continue;
            ...
            int targetInboxSize = targetSize[0] * targetSize[1] * targetSize[2];
            ...
            checkCudaError(cudaMemset(d_positiveCount, 0, sizeof(int)), "Memset d_positiveCount SPH");
            int targetBlocks = (targetInboxSize + threadsPerBlock - 1) / threadsPerBlock;
            countPositiveWeightsKernel<T><<<targetBlocks, threadsPerBlock>>>(d_remoteWeightSum, targetInboxSize, d_positiveCount);
            checkCudaError(cudaGetLastError(), "countPositiveWeightsKernel SPH");
            checkCudaError(cudaMemcpy(&targetNnz[targetRank], d_positiveCount, sizeof(int), cudaMemcpyDeviceToHost),
                            "Copying targetNnz SPH");
        }

        for (int i = 0; i < mesh.numRanks_; i++) targetOffset[i + 1] = targetOffset[i] + targetNnz[i];
        totalRemoteCount = targetOffset[mesh.numRanks_];

        if (totalRemoteCount > 0)
        {
            checkCudaError(cudaMalloc(&d_remoteRanksSPH, totalRemoteCount * sizeof(int)), "Allocating d_remoteRanksSPH");
            checkCudaError(cudaMalloc(&d_remoteIndicesSPH, totalRemoteCount * sizeof(uint64_t)), "Allocating d_remoteIndicesSPH");
            checkCudaError(cudaMalloc(&d_remoteWeightsSPH, totalRemoteCount * sizeof(T)), "Allocating d_remoteWeightsSPH");
            checkCudaError(cudaMalloc(&d_remoteWeightedVxSPH, totalRemoteCount * sizeof(T)), "Allocating d_remoteWeightedVxSPH");
            checkCudaError(cudaMalloc(&d_remoteWeightedVySPH, totalRemoteCount * sizeof(T)), "Allocating d_remoteWeightedVySPH");
            checkCudaError(cudaMalloc(&d_remoteWeightedVzSPH, totalRemoteCount * sizeof(T)), "Allocating d_remoteWeightedVzSPH");
        }

        // Pass 2: materialize compact remote contribution list on device.
        for (int targetRank = 0; targetRank < mesh.numRanks_; targetRank++)
        {
            if (targetRank == mesh.rank_ || targetNnz[targetRank] == 0) continue;
            ...
            checkCudaError(cudaMemset(d_remoteWeightSum, 0, targetInboxSize * sizeof(T)), "Memset d_remoteWeightSum pass2");
            checkCudaError(cudaMemset(d_remoteWeightedVelX, 0, targetInboxSize * sizeof(T)), "Memset d_remoteWeightedVelX pass2");
            checkCudaError(cudaMemset(d_remoteWeightedVelY, 0, targetInboxSize * sizeof(T)), "Memset d_remoteWeightedVelY pass2");
            checkCudaError(cudaMemset(d_remoteWeightedVelZ, 0, targetInboxSize * sizeof(T)), "Memset d_remoteWeightedVelZ pass2");

            checkCudaError(cudaMemcpy(d_targetLow, targetLow.data(), 3 * sizeof(int), cudaMemcpyHostToDevice),
                            "Copying remote target low to device pass2");
            checkCudaError(cudaMemcpy(d_targetHigh, targetHigh.data(), 3 * sizeof(int), cudaMemcpyHostToDevice),
                            "Copying remote target high to device pass2");
            checkCudaError(cudaMemcpy(d_targetSize, targetSize.data(), 3 * sizeof(int), cudaMemcpyHostToDevice),
                            "Copying remote target size to device pass2");

            accumulateSPHForTargetRankKernel<T><<<blocksPerGrid, threadsPerBlock>>>(
                d_x, d_y, d_z, d_vx, d_vy, d_vz, d_h, numParticles, mesh.gridDim_, mesh.Lmin_, mesh.Lmax_,
                d_targetLow, d_targetHigh, d_targetSize,
                d_remoteWeightSum, d_remoteWeightedVelX, d_remoteWeightedVelY, d_remoteWeightedVelZ);
            checkCudaError(cudaGetLastError(), "SPH remote rank aggregation kernel launch pass2");

            checkCudaError(cudaMemset(d_singleWriteOffset, 0, sizeof(int)), "Memset d_singleWriteOffset SPH");
            int targetBlocks = (targetInboxSize + threadsPerBlock - 1) / threadsPerBlock;
            packSPHTargetKernel<T><<<targetBlocks, threadsPerBlock>>>(
                targetRank,
                d_remoteWeightSum, d_remoteWeightedVelX, d_remoteWeightedVelY, d_remoteWeightedVelZ,
                targetInboxSize, targetOffset[targetRank], d_singleWriteOffset,
                d_remoteRanksSPH, d_remoteIndicesSPH,
                d_remoteWeightsSPH, d_remoteWeightedVxSPH, d_remoteWeightedVySPH, d_remoteWeightedVzSPH);
            checkCudaError(cudaGetLastError(), "packSPHTargetKernel");
        }

        // Build per-rank send counts from compact device list.
        int* d_sendCount = nullptr;
        checkCudaError(cudaMalloc(&d_sendCount, mesh.numRanks_ * sizeof(int)), "Allocating d_sendCount SPH");
        checkCudaError(cudaMemset(d_sendCount, 0, mesh.numRanks_ * sizeof(int)), "Memset d_sendCount SPH");
        if (totalRemoteCount > 0)
        {
            int cb = (totalRemoteCount + threadsPerBlock - 1) / threadsPerBlock;
            countRemoteByRankKernel<<<cb, threadsPerBlock>>>(d_remoteRanksSPH, totalRemoteCount, d_sendCount);
            checkCudaError(cudaGetLastError(), "countRemoteByRankKernel SPH");
        }
        checkCudaError(cudaMemcpy(mesh.send_count.data(), d_sendCount, mesh.numRanks_ * sizeof(int), cudaMemcpyDeviceToHost),
                        "Copying send_count SPH from device");
        cudaFree(d_sendCount);
    ...
    if (d_positiveCount) cudaFree(d_positiveCount);
    if (d_singleWriteOffset) cudaFree(d_singleWriteOffset);
    ...
    SphMsg<T>* d_sendSphPacked = nullptr;
    SphMsg<T>* d_recvSphPacked = nullptr;
    std::vector<SphMsg<T>> sendSphPacked, recvSphPacked;

    int sendTotal = mesh.send_disp[mesh.numRanks_];
    if (!useFullGpuPack)
    {
        sendSphPacked.resize(sendTotal);
        for (int i = 0; i < mesh.numRanks_; i++)
        {
            for (int j = mesh.send_disp[i]; j < mesh.send_disp[i + 1]; j++)
            {
                int localIdx    = j - mesh.send_disp[i];
                sendSphPacked[j] = SphMsg<T>{mesh.vdataSenderSPH[i].send_index[localIdx],
                                             mesh.vdataSenderSPH[i].send_weight[localIdx],
                                             mesh.vdataSenderSPH[i].send_weighted_vx[localIdx],
                                             mesh.vdataSenderSPH[i].send_weighted_vy[localIdx],
                                             mesh.vdataSenderSPH[i].send_weighted_vz[localIdx]};
            }
        }
    }

    MPI_Datatype sphMsgType;
    MPI_Type_contiguous(sizeof(SphMsg<T>), MPI_BYTE, &sphMsgType);
    MPI_Type_commit(&sphMsgType);

    if (useFullGpuPack)
    {
        if (sendTotal > 0)
        {
            checkCudaError(cudaMalloc(&d_sendSphPacked, sendTotal * sizeof(SphMsg<T>)), "Allocating d_sendSphPacked");

            int* d_sendDisp  = nullptr;
            int* d_writeOff  = nullptr;
            checkCudaError(cudaMalloc(&d_sendDisp, mesh.numRanks_ * sizeof(int)), "Allocating d_sendDisp SPH");
            checkCudaError(cudaMalloc(&d_writeOff, mesh.numRanks_ * sizeof(int)), "Allocating d_writeOff SPH");
            checkCudaError(cudaMemcpy(d_sendDisp, mesh.send_disp.data(), mesh.numRanks_ * sizeof(int), cudaMemcpyHostToDevice),
                           "Copying send_disp SPH to device");
            checkCudaError(cudaMemset(d_writeOff, 0, mesh.numRanks_ * sizeof(int)), "Memset d_writeOff SPH");

            if (totalRemoteCount > 0)
            {
                int pb = (totalRemoteCount + threadsPerBlock - 1) / threadsPerBlock;
                packSphByRankKernel<T><<<pb, threadsPerBlock>>>(
                    d_remoteRanksSPH, d_remoteIndicesSPH, d_remoteWeightsSPH,
                    d_remoteWeightedVxSPH, d_remoteWeightedVySPH, d_remoteWeightedVzSPH,
                    totalRemoteCount, d_sendDisp, d_writeOff, d_sendSphPacked);
                checkCudaError(cudaGetLastError(), "packSphByRankKernel");
            }
        ...
        if (recvTotal > 0)
        {
            checkCudaError(cudaMalloc(&d_recvSphPacked, recvTotal * sizeof(SphMsg<T>)), "Allocating d_recvSphPacked");
        }

        MPI_Alltoallv(d_sendSphPacked, mesh.send_count.data(), mesh.send_disp.data(), sphMsgType,
                  d_recvSphPacked, mesh.recv_count.data(), mesh.recv_disp.data(), sphMsgType,
                      MPI_COMM_WORLD);

        if (d_sendSphPacked) cudaFree(d_sendSphPacked);
        if (d_remoteRanksSPH) cudaFree(d_remoteRanksSPH);
        if (d_remoteIndicesSPH) cudaFree(d_remoteIndicesSPH);
        if (d_remoteWeightsSPH) cudaFree(d_remoteWeightsSPH);
        if (d_remoteWeightedVxSPH) cudaFree(d_remoteWeightedVxSPH);
        if (d_remoteWeightedVySPH) cudaFree(d_remoteWeightedVySPH);
        if (d_remoteWeightedVzSPH) cudaFree(d_remoteWeightedVzSPH);
    }
    else
    {
        recvSphPacked.resize(recvTotal);

        MPI_Alltoallv(sendSphPacked.data(), mesh.send_count.data(), mesh.send_disp.data(), sphMsgType,
                  recvSphPacked.data(), mesh.recv_count.data(), mesh.recv_disp.data(), sphMsgType,
                      MPI_COMM_WORLD);

        if (recvTotal > 0)
        {
            checkCudaError(cudaMalloc(&d_recvSphPacked, recvTotal * sizeof(SphMsg<T>)), "Allocating d_recvSphPacked");
            checkCudaError(cudaMemcpy(d_recvSphPacked, recvSphPacked.data(), recvTotal * sizeof(SphMsg<T>), cudaMemcpyHostToDevice),
                           "Copying packed SPH recv to device");
        }
    }

    MPI_Type_free(&sphMsgType);
    ...
    if (recvTotal > 0)
    {
        ...
        int recvBlocks = (recvTotal + threadsPerBlock - 1) / threadsPerBlock;
        if (d_recvSphPacked)
            accumulateSPHPackedKernel<T><<<recvBlocks, threadsPerBlock>>>(
                d_recvSphPacked, recvTotal,
                d_meshWeightSum, d_meshWeightedVelX, d_meshWeightedVelY,
                d_meshWeightedVelZ);
        else
        ...
        if (d_recvSphPacked) cudaFree(d_recvSphPacked);
        if (d_recvIndices) cudaFree(d_recvIndices);
        if (d_recvWeights) cudaFree(d_recvWeights);
        if (d_recvWeightedVx) cudaFree(d_recvWeightedVx);
        if (d_recvWeightedVy) cudaFree(d_recvWeightedVy);
        if (d_recvWeightedVz) cudaFree(d_recvWeightedVz);
    }
                                               ...
                                               int powerDim)
{
    ...
    {
        // Host-staged path uses a single packed exchange (CellAvgMsg),
        // so separate per-field send buffers are not needed.
    }
    ...
    CellAvgMsg<T>* d_recvCellAvgPacked = nullptr;
    std::vector<CellAvgMsg<T>> sendCellAvgPacked, recvCellAvgPacked;
    ...
    {
        int sendTotal = mesh.send_disp[mesh.numRanks_];
        sendCellAvgPacked.resize(sendTotal);
        for (int i = 0; i < mesh.numRanks_; i++)
            for (int j = mesh.send_disp[i]; j < mesh.send_disp[i + 1]; j++)
            {
                int local          = j - mesh.send_disp[i];
                sendCellAvgPacked[j] = CellAvgMsg<T>{mesh.vdataSenderCellAvg[i].send_index[local],
                                                     mesh.vdataSenderCellAvg[i].send_vx[local],
                                                     mesh.vdataSenderCellAvg[i].send_vy[local],
                                                     mesh.vdataSenderCellAvg[i].send_vz[local]};
            }

        recvCellAvgPacked.resize(recvTotal);
        MPI_Datatype cellAvgMsgType;
        MPI_Type_contiguous(sizeof(CellAvgMsg<T>), MPI_BYTE, &cellAvgMsgType);
        MPI_Type_commit(&cellAvgMsgType);

        MPI_Alltoallv(sendCellAvgPacked.data(), mesh.send_count.data(), mesh.send_disp.data(), cellAvgMsgType,
                      recvCellAvgPacked.data(), mesh.recv_count.data(), mesh.recv_disp.data(), cellAvgMsgType,
                      MPI_COMM_WORLD);
        MPI_Type_free(&cellAvgMsgType);

        if (recvTotal > 0)
        {
            checkCudaError(cudaMalloc(&d_recvCellAvgPacked, recvTotal * sizeof(CellAvgMsg<T>)), "d_recvCellAvgPacked");
            checkCudaError(cudaMemcpy(d_recvCellAvgPacked, recvCellAvgPacked.data(), recvTotal * sizeof(CellAvgMsg<T>), cudaMemcpyHostToDevice),
                           "cp recv packed cavg");
        }
    }
    ...
    if (recvTotal > 0)
    {
        ...
        if (d_recvCellAvgPacked)
            accumulateCellAvgPackedKernel<T><<<rb, threadsPerBlock>>>(
                d_recvCellAvgPacked, recvTotal,
                d_velXSum, d_velYSum, d_velZSum, d_cellCount);
        else
        ...
        if (d_recvCellAvgPacked) cudaFree(d_recvCellAvgPacked);
        if (d_recvIndices) cudaFree(d_recvIndices);
        if (d_recvVx) cudaFree(d_recvVx);
        if (d_recvVy) cudaFree(d_recvVy);
        if (d_recvVz) cudaFree(d_recvVz);
